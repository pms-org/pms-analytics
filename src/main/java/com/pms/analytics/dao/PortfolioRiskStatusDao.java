...
public class PortfolioRiskStatusDao {
    ...
    public boolean tryAdvisoryLock(UUID portfolioId) {
        return Boolean.TRUE.equals(
                jdbcTemplate.queryForObject(
                        "SELECT pg_try_advisory_xact_lock(hashtext('RISK:' || ?))::boolean",
                        Boolean.class,
    ...
    public void updateLastComputed(UUID portfolioId) {
        jdbcTemplate.update("""
            INSERT INTO analytics_portfolio_risk_status (portfolio_id, last_computed_at)
            VALUES (?, now())
